"""
Assign 03 - Christian Lopez.

Credits:
    * Worked with Sean Kruse throughout project.
    * https://www.programiz.com/dsa/floyd-warshall-algorithm
    * https://bradfieldcs.com/algos/graphs/dijkstras-algorithm/#:~:text
    Dijkstra's%20algorithm%20uses%20a%20priority,of%20vertices%20sorted%20by%20distance.
...
def floyd(W):
    ...
    # orignial ideas were the following however I think it isnt the right way to do it.
    # dist = W
    # dist = W.copy()
    dist = [row[:] for row in W]
    # Finds the number of verticies
    numVerticies = len(W)
    # Traverse through every possible path from vetex i,j,k
    for x in range(numVerticies):
        for y in range(numVerticies):
            for z in range(numVerticies):
...
def dijkstra_w_pri_queue(W, sv):
    ...
    pq = PriorityQueue()
    # Create a list of distances initialized to infinity, with one element for each vertex in the graph
    distance = [INF] * len(W)
    # Set the distance to the starting vertex to 0
    distance[sv] = 0
    # Add the start vertex to the priority queue with a priority of 0
    pq.put((0, sv))
    # While the priority queue is not empty, get the item with the shortest distance or priority
    while not pq.empty():
        # the underscore says to ignore the priority (priority, item)
        # get the vertex with the shortest distance
        (_, minDistVertex) = pq.get()
        # Visit each neighbor of the current vertex and update its distance
        for vertex in range(len(W)):
            edgeWeight = W[minDistVertex][vertex]
            # do nothing if the vertex is looking at itself
            if edgeWeight != 0:
                # update the distance to the current vertex if it is shorter than the minDistVertex
                if distance[minDistVertex] + edgeWeight < distance[vertex]:
                    distance[vertex] = distance[minDistVertex] + edgeWeight
                    pq.put((distance[vertex], vertex))
    return distance


def dijkstra_w_array(W, sv):
    """Dijkstra's using an array w/ adj matrix W and sv as starting vertex."""
    # Create a list of distances initialized to infinity, with one element for each vertex in the graph
    dist = [INF] * len(W)
    # Set the distance to the starting vertex to 0
    dist[sv] = 0
        ...
        for j in range(len(W)):
            if j not in visited:
                if dist[j] < min_dist:
                    min_dist = dist[j]
                    min_vertex = j
        # If no unvisited vertex was found, break out of the loop
        if min_vertex == -1:
            break
        # add the vertex to say it has been visited
        visited.add(min_vertex)
        # j is the vertex neighbor
        for j in range(len(W)):
            # do nothing if it is checking the vertex against itself
            if W[min_vertex][j] != 0:
                # if an edge exists check if distance between tge min vertex + edge weight is
                # less than current distance to j.
                # update distance to j
                if dist[min_vertex] + W[min_vertex][j] < dist[j]:
                    dist[j] = dist[min_vertex] + W[min_vertex][j]
    return dist


def assign03_main():
    """Demonstrate the functions, starting with creating the graph."""
    g = adjMatFromFile("graph_50verts.txt")
    if len(g) <= 20:
