...
Directions:
    ...
    * NOTE: Add a docstring for each function, and that a reasonable
...
def linearSearch(list_of_items, item_sought):
    """Preforms a linear search when given array and number."""
    num_comparisons = 0
    item_found = False
...
def binarySearch(list_of_items, item_sought):
    """Preforms a binary search when given array and number."""
    num_comparisons = 0
    item_found = False
    start_time = time.time()
    ...
    # num_comparisons and item_found AS NEEDED
    lowElement = 0
    highElement = len(list_of_items) - 1
    while lowElement <= highElement:
        num_comparisons += 1
        midElement = (lowElement + highElement) // 2
        if (list_of_items[midElement] == item_sought):
            item_found = True
            break
        elif (item_sought < list_of_items[midElement]):
